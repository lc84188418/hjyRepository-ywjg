...
//    @RequiresPermissions({"approval:add"})
    ...
    public CommonResult syntheticalSelect(@RequestBody THallQueue tHallQueue) throws FebsException {
        try {
            //
     ...
     * 有信息后访问同步库数据
     */
    @PostMapping("/list/info/getTbkData")
    public Map<String, Object> getTbkData(@RequestBody THallQueue tHallQueue) throws SQLException, ConnectException {
        Map<String, Object> map = new HashMap<>();
        try {
            /**
             * 先判断是否同步库连接有问题，若有问题，需返回相应提示信息
             */
            map = tListInfoService.getTbkData(tHallQueue);
            ...
            map.put("exception","449同步库连接出错，请联系维护人员排查");
            return map;
        }
    }
}