...
import com.hjy.system.entity.ReUserPerms;
...
public interface TSysUserMapper {
    ...
    //该用户所有3级菜单
    List<TSysPerms> selectQuickSetByUser(@Param("fkUserId")String fkUserId);
    //该用户已设置的快捷菜单
    List<String> selectUserPermsByUser(@Param("fkUserId")String fkUserId);

    int deleteUserPermsByUserId(@Param("fkUserId")String fkUserId);

    int addUserPermsByList(@Param("idList")List<ReUserPerms> userPermsList);
    //该用户已设置的快捷菜单信息
    List<TSysPerms> selectPermsByUser(@Param("fkUserId")String userId);
}