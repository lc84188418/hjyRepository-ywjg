...
public class CardFileUtil {
    ...
    //批量制证完成添加
    public static String MakeCardShareFileCompletBatch(List<TSyntheticalMakecard> list) {
        List<String> resultList = new ArrayList<>();
        for(TSyntheticalMakecard tHallMakecard :list){
            if(tHallMakecard.getBIdcard() == null){
                return null;
            }
            //先在首行添加制证完成的信息
    ...
    //制证领取、弃用
    public static String MakeCardShareFileDelBatch(List<TSyntheticalMakecard> list) {
        /**
         * 先读取出文件中的内容
         */
        List<String> oldContext = new ArrayList<>();
        //再读取文件
        FileInputStream in = null;
        try {
            in = new FileInputStream(CARD_FILE_PATH);
